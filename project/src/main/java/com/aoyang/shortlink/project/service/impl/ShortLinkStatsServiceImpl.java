...
import com.aoyang.shortlink.project.dto.req.ShortLinkGroupStatsAccessRecordReqDTO;
import com.aoyang.shortlink.project.dto.req.ShortLinkStatsAccessRecordReqDTO;
import com.aoyang.shortlink.project.dto.req.ShortLinkStatsReqDTO;
import com.aoyang.shortlink.project.dto.resp.*;
import com.aoyang.shortlink.project.service.ShortLinkStatsService;
...
public class ShortLinkStatsServiceImpl implements ShortLinkStatsService {
    ...
    public IPage<ShortLinkStatsAccessRecordRespDTO> shortLinkStatsAccessRecord(ShortLinkStatsAccessRecordReqDTO requestParam) {
        ...
        // 新建一个Set集合用于存储已经被标识过的user
        Set<String> identifiedUserSet = new HashSet<>();
        // 对每条记录进行遍历
        actualResult.getRecords().forEach(each -> {
            String uvType = "老访客";
            if (!identifiedUserSet.contains(each.getUser())) {
                uvType = uvTypeList.stream()
                        .filter(item -> Objects.equals(each.getUser(), item.get("user")))
                        .findFirst()
                        .map(item -> item.get("UvType"))
                        .map(Object::toString)
                        .orElse("老访客");
                // 如果该用户第一次出现，则将其记录至Set中，表示已经被标记
                identifiedUserSet.add(each.getUser());
            }
